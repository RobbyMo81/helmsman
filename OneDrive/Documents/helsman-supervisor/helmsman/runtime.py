...
VENV = "venv"
ART = pathlib.Path("artifacts"); LOG = ART/"logs"; REP = ART/"reports"
...
def ensure_dirs():
    for d in [ART, LOG, REP]: d.mkdir(parents=True, exist_ok=True)
...
def health(cfg: Dict[str, Any]) -> Health:
    ...
    try:
        import importlib; torch = importlib.import_module("torch")
        torch_ok = True; cuda = bool(getattr(torch, "cuda", None) and torch.cuda.is_available())
    except Exception as e:
        notes.append(f"torch import failed: {e}")
    requirements_ok = bool(cfg.get("requirements") and pathlib.Path(cfg["requirements"]).exists())
    ports = {"streamlit": port_is_free(int(cfg["streamlit"]["port"]))}
    h = Health(time.time(), sys.version.split(" ")[0], has_venv, torch_ok, cuda, requirements_ok, ports, notes)
    write_json(REP/"health.json", asdict(h)); return h
...
def _launch(name: str, cmd: List[str], log_path: pathlib.Path) -> Optional[ProcInfo]:
    ...
    p = subprocess.Popen(cmd, stdout=log_f, stderr=subprocess.STDOUT, cwd=str(pathlib.Path.cwd()))
    info = ProcInfo(name=name, pid=p.pid, cmd=cmd, log_file=str(log_path), start_time=time.time(), proc=p, log_handle=log_f)
    PROCS[name] = info
    return info

def run_streamlit(cfg: Dict[str, Any]):
    entry = pathlib.Path(cfg["streamlit"]["entry"]); port = str(cfg["streamlit"]["port"])
    if not entry.exists(): raise FileNotFoundError(f"No Streamlit entry: {entry}")
    if not port_is_free(int(port)): raise RuntimeError(f"Port {port} not free")
...
def run_torch(cfg: Dict[str, Any], infile: Optional[str], device: Optional[str], max_batch: int):
    entry = pathlib.Path(cfg["torch_app"]["entry"])
    if not entry.exists(): raise FileNotFoundError(f"No Torch entry: {entry}")
    cmd = [_py(), str(entry)]
    if infile: cmd += ["--infile", infile]
    if device: cmd += ["--device", device]
    if max_batch: cmd += ["--max-batch", str(max_batch)]
