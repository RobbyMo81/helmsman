...
def test_list_models(memory_store):
    ...
    memory_store.save_model_metadata(
        name="model_a", file_path="a.pth", architecture="NetA"
    )
    memory_store.save_model_metadata(
        name="model_b", file_path="b.pth", architecture="NetB"
    )
...
def test_delete_model(memory_store):
    ...
    memory_store.save_model_metadata(
        name=model_name, file_path=f"models/{model_name}.pth", architecture="TestNet"
    )
...
def test_update_training_session(memory_store):
    ...
    memory_store.update_training_session(
        job_id, status="completed", progress=100, error_message="None"
    )
    session = memory_store.get_training_session(job_id)
