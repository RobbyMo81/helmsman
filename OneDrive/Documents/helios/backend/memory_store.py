...
class MemoryStore:
        ...
        with self.lock:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (name, file_path, architecture, version, timestamp, timestamp, metadata_json))
        ...
        with self.lock:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (job_id, model_name, 'started', timestamp, config_json, timestamp, timestamp))
        ...
        with self._get_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                if prediction['actual_outcome']:
                    prediction['actual_outcome'] = json.loads(prediction['actual_outcome'])
    ...
    def store_enhanced_journal_entry(self,
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (model_name, session_id, event_type, json.dumps(event_data), 
                  confidence_score, success_metric, context_hash))
            ...
            logger.info(f"Stored enhanced journal entry {entry_id} for model {model_name}")
            return entry_id
    ...
    def store_knowledge_fragment(self,
        ...
        with self._get_connection() as conn:
            ...
            logger.info(f"Stored knowledge fragment {fragment_id} for model {model_name}")
            return fragment_id
    ...
    def store_performance_metric(self,
        ...
        with self._get_connection() as conn:
            ...
            logger.info(f"Stored performance metric {metric_name} for model {model_name}: {metric_value}")
            return metric_id
    ...
    def compact_memory(self,
                      archive_days: int = 30,
                      relevance_threshold: float = 0.1) -> Dict[str, int]:
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            tables = ['enhanced_journal', 'knowledge_fragments', 'performance_metrics', 'memory_operations']
            for table in tables:
