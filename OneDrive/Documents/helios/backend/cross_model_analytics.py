...
class CrossModelAnalytics:
    ...
    def analyze_model_performance(self, model_name: str, 
                                 days_back: int = 30) -> ModelPerformanceMetrics:
        """
    ...
    def compare_models(self, model_names: List[str], 
                      comparison_type: str = "comprehensive") -> CrossModelComparison:
        """
        ...
        efficiency_ranking = sorted(
            [(name, metrics.efficiency_score) for name, metrics in model_metrics.items()],
            key=lambda x: x[1],
            reverse=True
        )
    ...
    def generate_ensemble_recommendations(self, 
                                        ...
                                        target_metric: str = "loss") -> List[EnsembleRecommendation]:
        """
        ...
        """
        logger.info(f"Generating ensemble recommendations for {len(model_names)} models")
        
        # Analyze all models
        model_metrics = {
            name: self.analyze_model_performance(name) 
        ...
        if len(best_performers) >= 2:
            ...
            rec = EnsembleRecommendation(
                ...
                weights=weights,
                expected_performance=self._estimate_ensemble_performance(best_performers, weights),
        ...
        if len(diverse_models) >= 2:
            ...
            rec = EnsembleRecommendation(
                ...
                weights=weights,
                expected_performance=self._estimate_ensemble_performance(diverse_models, weights),
        ...
        if len(stable_models) >= 2:
            ...
            rec = EnsembleRecommendation(
                ...
                weights=weights,
                expected_performance=self._estimate_ensemble_performance(stable_models, weights),
    ...
    def get_performance_matrix(self, model_names: List[str]) -> Dict[str, Any]:
        ...
        for i, metric in enumerate(matrix_data["metrics"]):
            values = [(model_names[j], matrix_data["data"][i][j]) 
    ...
    def _get_training_history(self, model_name: str, days_back: int) -> List[Dict[str, Any]]:
    ...
    def _analyze_convergence(self, training_history: List[Dict[str, Any]]) -> Optional[int]:
        ...
        if total_epochs:
            return int(np.median(total_epochs) * 0.7)  # Assume convergence at 70% of training
    ...
    def _calculate_stability_score(self, training_history: List[Dict[str, Any]]) -> float:
        ...
        losses = [h.get('final_loss', float('inf')) for h in training_history 
    ...
    def _calculate_efficiency_score(self, training_history: List[Dict[str, Any]]) -> float:
    ...
    def _calculate_recommendation_scores(self, 
                                       model_metrics: Dict[str, ModelPerformanceMetrics]) -> Dict[str, float]:
        """Calculate overall recommendation scores for models"""
        scores = {}
        
        for name, metrics in model_metrics.items():
            # Weighted combination of performance, stability, and efficiency
            performance_score = 1.0 / (1.0 + metrics.best_loss) if metrics.best_loss != float('inf') else 0.0
            stability_score = metrics.stability_score
            efficiency_score = metrics.efficiency_score
    ...
    def _assess_ensemble_potential(self, 
                                 model_metrics: Dict[str, ModelPerformanceMetrics]) -> float:
        """Assess how well the models would work together in an ensemble"""
        if len(model_metrics) < 2:
            return 0.0
        
        # Diversity factor - models should have different strengths
        performance_scores = [1.0 / (1.0 + m.best_loss) if m.best_loss != float('inf') else 0.0 
    ...
    def _select_best_performers(self, model_metrics: Dict[str, ModelPerformanceMetrics], 
                              top_n: int = 3) -> Dict[str, ModelPerformanceMetrics]:
    ...
    def _select_diverse_models(self, 
                             model_metrics: Dict[str, ModelPerformanceMetrics]) -> Dict[str, ModelPerformanceMetrics]:
    ...
    def _select_stable_models(self, 
                            model_metrics: Dict[str, ModelPerformanceMetrics]) -> Dict[str, ModelPerformanceMetrics]:
    ...
    def _calculate_optimal_weights(self, models: Dict[str, ModelPerformanceMetrics]) -> List[float]:
    ...
    def _calculate_diversity_weights(self, models: Dict[str, ModelPerformanceMetrics]) -> List[float]:
    ...
    def _calculate_stability_weights(self, models: Dict[str, ModelPerformanceMetrics]) -> List[float]:
    ...
    def _estimate_ensemble_performance(self, models: Dict[str, ModelPerformanceMetrics], 
    ...
    def _calculate_overall_trends(self, model_trends: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_improvement_rate": np.mean(improvement_rates) if improvement_rates else 0.0,
            "average_consistency": np.mean(consistency_scores) if consistency_scores else 0.0,
            "total_models_analyzed": len(model_trends)
        }
    
    def _generate_trend_insights(self, model_trends: Dict[str, Dict[str, Any]], 
                               overall_trends: Dict[str, Any]) -> List[str]:
        ...
        if total_valid > 0:
            if improving_count > total_valid * 0.6:
                insights.append("Most models are showing improvement in recent training sessions")
            elif declining_count > total_valid * 0.6:
                insights.append("Concerning: Most models are showing declining performance")
            elif stable_count > total_valid * 0.6:
                insights.append("Model performance is generally stable with minimal changes")
            else:
                insights.append("Mixed trends: Models showing varied performance patterns")
        
        # Improvement rate insights
        avg_improvement = overall_trends.get("average_improvement_rate", 0.0)
        if avg_improvement > 0.1:
            insights.append(f"Strong improvement trend: Average {avg_improvement:.1%} performance gain")
        elif avg_improvement < -0.1:
            insights.append(f"Performance decline: Average {abs(avg_improvement):.1%} loss in performance")
        
        # Consistency insights
        avg_consistency = overall_trends.get("average_consistency", 0.0)
        if avg_consistency > 0.8:
            insights.append("High consistency: Models showing reliable performance patterns")
        elif avg_consistency < 0.5:
            insights.append("Low consistency: Models showing unpredictable performance variations")
